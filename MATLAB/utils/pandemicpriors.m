function [Xst, Yst, xd, yd] = pandemicpriors(X,Y,Yraw,nAR,constant,delta,lambda,tau,epsilon,phi,covid_periods,coper)
...
% Co-persistence / dummy-initial-observation prior (Sims-Zha 1998):
% coper > 0 activates it; coper = 0 reproduces the standard specification.
% Passed explicitly by the caller (see the settings block in run_pandemic_priors.m).

%% Check if phi is a scalar or a vector
if length(phi) ==1; phi = ones(1,covid_periods)*phi; end
...
%% Construct co-persistence / dummy-initial-observation prior (Sims-Zha 1998)
% A single extra dummy observation built from the average of the initial nAR
% (pre-sample) observations. It couples the intercept to the common level and
% favours a shared stochastic trend across variables. coper controls the
% tightness (smaller coper -> tighter prior). Off by default (coper=0).
yd3 = []; xd3 = [];
if coper>0
    y0  = mean(Yraw(1:nAR,:),1);          % 1 x n average of the initial conditions
    yd3 = y0./coper;                       % 1 x n LHS dummy
    if constant == 1
        xd3 = [kron(ones(1,nAR),yd3) 1/coper zeros(1,covid_periods)];
    else
        xd3 = [kron(ones(1,nAR),yd3) zeros(1,covid_periods)];
    end
end

yd=[yd1;yd2;yd3]; xd=[xd1;xd2;xd3]; Yst = [Y; yd]; Xst = [X; xd];
